...
namespace RevitProjectDataAddin
{
    public partial class 梁配筋施工図 : Window
    {
        ...
        private Ellipse DrawDotMm_Rec(Canvas c, WCTransform T, GridBotsecozu owner,
                                     double wx, double wy, double rMm = 60, // bán kính theo mm (DXF)
                                     Brush fill = null, Brush stroke = null, double strokePx = 0.8,
                                     string layer = "MARK")
        {
            // 1) Ghi CIRCLE vào Scene để DXF có chấm tròn thật
            SceneFor(owner).Add(new DxfCircle(wx, wy, rMm, layer));

            // 2) Vẽ lên UI: ellipse theo pixel (tính sizePx từ rMm để nhìn tương xứng)
            double sizePx = Math.Max(3.0, 2.0 * rMm * T.Scale);
     ...
     LineRole role, GridBotsecozu owner, int? index = null)
        ...
        private void Redraw(Canvas canvas, GridBotsecozu item)
        {
            ...
            // offsets theo trục (giữ nguyên vòng for trục như bạn có)
        ...
        // ===== XÂY DỰNG HÌNH HỌC DXF: trả về cả LINE + TEXT =====
        private (List<(double x1, double y1, double x2, double y2)> lines, List<DxfText> texts, string fileKey)
          BuildDxfGeometry_ByData(GridBotsecozu item)
        {
            var k = _projectData?.Kihon;
            if (k == null || _currentSecoList == null)
                return (new List<(double, double, double, double)>(), new List<DxfText>(), "untitled");
            ...
            if (names.Count < 2 || spans.Count < names.Count - 1)
                return (new List<(double, double, double, double)>(), new List<DxfText>(),
                        $"{_currentSecoList.階を選択}_{_currentSecoList.通を選択}");

            double W = spans.Take(names.Count - 1).Sum();
            double H = Math.Max(1, W * 0.4);
            ...
            var key = $"{_currentSecoList.階を選択}_{_currentSecoList.通を選択}";
      ...
      bool flipY = true)
        ...
        private void ShowOrInitBeamRebarData()
        {
            // Lấy beam đầu tiên trong danh sách (hoặc chọn beam phù hợp theo ngữ cảnh)
            var beam = _projectData?.リスト?.梁リスト?
                           .FirstOrDefault()?.梁?
                           .FirstOrDefault();

            if (beam == null)
            {
                MessageBox.Show("Không tìm thấy dữ liệu 梁 trong ProjectData.", "Debug 梁配筋");
                return;
            }

            // Đảm bảo 梁の配置 tồn tại
            if (beam.梁の配置 == null)
            {
                beam.梁の配置 = new Z梁の配置();
            }
            ...
            // --- Gán giá trị mặc định nếu null hoặc rỗng ---
            // 端部1
            z.端部1上筋本数 = string.IsNullOrEmpty(z.端部1上筋本数) ? "1" : z.端部1上筋本数;
            z.端部1上宙1 = string.IsNullOrEmpty(z.端部1上宙1) ? "1" : z.端部1上宙1;
            z.端部1上宙2 = string.IsNullOrEmpty(z.端部1上宙2) ? "1" : z.端部1上宙2;
            z.端部1下宙2 = string.IsNullOrEmpty(z.端部1下宙2) ? "1" : z.端部1下宙2;
            z.端部1下宙1 = string.IsNullOrEmpty(z.端部1下宙1) ? "1" : z.端部1下宙1;
            z.端部1下筋本数 = string.IsNullOrEmpty(z.端部1下筋本数) ? "1" : z.端部1下筋本数;

            // 中央
            z.中央上筋本数 = string.IsNullOrEmpty(z.中央上筋本数) ? "1" : z.中央上筋本数;
            z.中央上宙1 = string.IsNullOrEmpty(z.中央上宙1) ? "1" : z.中央上宙1;
            z.中央上宙2 = string.IsNullOrEmpty(z.中央上宙2) ? "1" : z.中央上宙2;
            z.中央下宙2 = string.IsNullOrEmpty(z.中央下宙2) ? "1" : z.中央下宙2;
            z.中央下宙1 = string.IsNullOrEmpty(z.中央下宙1) ? "1" : z.中央下宙1;
            z.中央下筋本数 = string.IsNullOrEmpty(z.中央下筋本数) ? "1" : z.中央下筋本数;

            // 端部2
            z.端部2上筋本数 = string.IsNullOrEmpty(z.端部2上筋本数) ? "1" : z.端部2上筋本数;
            z.端部2上宙1 = string.IsNullOrEmpty(z.端部2上宙1) ? "1" : z.端部2上宙1;
            z.端部2上宙2 = string.IsNullOrEmpty(z.端部2上宙2) ? "1" : z.端部2上宙2;
            z.端部2下宙2 = string.IsNullOrEmpty(z.端部2下宙2) ? "1" : z.端部2下宙2;
            z.端部2下宙1 = string.IsNullOrEmpty(z.端部2下宙1) ? "1" : z.端部2下宙1;
            z.端部2下筋本数 = string.IsNullOrEmpty(z.端部2下筋本数) ? "1" : z.端部2下筋本数;

            // --- Hiển thị tất cả dữ liệu ---
            string msg =
                $"【端部1】上筋本数={z.端部1上筋本数}, 上宙1={z.端部1上宙1}, 上宙2={z.端部1上宙2}, 下宙1={z.端部1下宙1}, 下宙2={z.端部1下宙2}, 下筋本数={z.端部1下筋本数}\n\n" +
                $"【中央】  上筋本数={z.中央上筋本数}, 上宙1={z.中央上宙1}, 上宙2={z.中央上宙2}, 下宙1={z.中央下宙1}, 下宙2={z.中央下宙2}, 下筋本数={z.中央下筋本数}\n\n" +
                $"【端部2】上筋本数={z.端部2上筋本数}, 上宙1={z.端部2上宙1}, 上宙2={z.端部2上宙2}, 下宙1={z.端部2下宙1}, 下宙2={z.端部2下宙2}, 下筋本数={z.端部2下筋本数}";

            MessageBox.Show(msg, "梁の配置データ");
        }
        //private (double up, double down, double left, double right) GetOffsetsByPosition(string kai, string tsu, string name)
        // Trả về 4 giá trị của SEGMENT ĐẦU TIÊN theo Kai/Tsu đang chọn.
        // G0: số lấy từ "梁の符号" (G0 -> 0, G12 -> 12, không parse được -> 0)
        // 上側/下側/梁の段差: parse double từ chuỗi (fallback 300/300/-200)
